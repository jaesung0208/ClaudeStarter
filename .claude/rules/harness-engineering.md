...
1. **단위 테스트**: `pytest app/backend/tests/test_{파일명}.py` (변경된 파일 관련 테스트)
2. **통합 테스트**: `pytest app/backend/tests/` (전체, 가능한 경우)
3. **Linter**: `pnpm lint` (프론트엔드 파일 변경 시)
4. **Syntax 확인**: posttooluse hook이 자동으로 검증 (Python syntax)
...
## 원칙 6: ISMS·시큐어코딩 (보안 필수 준수)

> AI가 생성하는 코드도 ISMS 인증 기준을 동일하게 준수한다.
> 상세 체크리스트: `.claude/skills/secure-coding.md`

### AI 행동 규칙 (항상 적용)

**절대 금지 — 사용자 명시 요청 없이 수행 불가**:
- DDL 직접 실행 (`CREATE TABLE`, `DROP TABLE`, `ALTER TABLE`, `TRUNCATE`) — pretooluse hook 자동 차단 (Level 2)
- 코드에 시크릿·API Key·DB 접속 정보 하드코딩 — posttooluse hook 자동 감지
- 요청 범위 외 파일 삭제·디렉토리 구조 변경
- API 응답·로그에 비밀번호·주민번호 등 민감정보 포함

**항상 준수**:
- 환경변수 주입 패턴 사용 (`os.environ.get("KEY")` / `process.env.KEY`)
- DB 쿼리는 Parameterized Query 또는 ORM 경유 (SQL Injection 방지)
- 에러 핸들러에서 스택 트레이스 응답 제거 (프로덕션)
- 보안 관련 Task에 `secure-coding` 스킬 자동 참조

**DDL 필요 시**: 즉흥적인 CLI 직접 실행 금지. 마이그레이션 스크립트(alembic 등) 경유 필수.

| 원칙 6 항목 | 의무 행동 | Hook 자동화 |
|------------|----------|-----------| 
| 하드코딩 금지 | 시크릿·URL 환경변수 분리 | posttooluse 자동 감지 |
| DDL 임의 실행 금지 | 마이그레이션 스크립트 경유 | pretooluse 차단 (Level 2) |
| 민감정보 노출 금지 | 응답 스키마에서 필드 exclude | posttooluse 자동 감지 |
| SQL Injection 방지 | Parameterized Query / ORM | 코드 리뷰 시 확인 |
