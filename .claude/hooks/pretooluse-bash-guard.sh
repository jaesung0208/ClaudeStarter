...
if echo "$COMMAND" | grep -qE '^\s*cd\s+[^\s&;]+\s*&&'; then
  ...
  block "[CLAUDE.md Bash 명령 규칙] 디렉토리 체이닝(cd /path && ...)은 금지됩니다.
...
# ── 규칙 7: DDL 직접 실행 차단 (AI 행동 규칙) ───────────────────────
# psql -c "DROP TABLE ..." 형태의 위험한 DDL 직접 실행 차단
# SETUP.sh에서 설정한 보안 수준(Level 2)일 때 적용
_DDL_BLOCK="false"
if [ -f ".claude/tmp/security-level.conf" ]; then
  _DDL_BLOCK=$(grep 'DDL_BLOCK' ".claude/tmp/security-level.conf" 2>/dev/null | cut -d= -f2 || echo "false")
fi

if [ "$_DDL_BLOCK" = "true" ]; then
  if echo "$COMMAND" | grep -qiE \
    '(psql|mysql|sqlite3|snowsql).+(-c|--command).+(CREATE TABLE|DROP TABLE|ALTER TABLE|TRUNCATE)'; then
    RULE_ID="ddl-direct"
    block "[CLAUDE.md 1-2 · secure-coding §5] DB DDL 직접 실행이 차단됩니다 (보안 Level 2).
  CREATE / DROP / ALTER / TRUNCATE 는 CLI 직접 실행 금지.
  → 마이그레이션 스크립트(alembic 등)를 사용하거나 사용자에게 명시적 허가를 받으세요."
  fi
fi
