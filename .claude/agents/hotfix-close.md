...
2. PR 생성 (hotfix → **main**)
3. 경량 코드 리뷰 (변경 파일만)
4. 타겟 검증 ($BACKEND_TEST_CMD + 해당 페이지/API만)
...
- `$BACKEND_TEST_CMD` (project-spec.conf 참조; 컨테이너 사용 시 `docker compose exec backend $BACKEND_TEST_CMD`)
...
**자동→수동 전환 조건** (**test-checklist skill** "자동 검증 전제 조건" 섹션 기준):
- Docker 미실행: `⬜ Docker 미실행으로 자동 검증 미수행` 기록 후 수동 항목으로 전환
- Playwright 미설치: `⬜ Playwright 미설치 — 수동 UI 검증 필요` 기록
...
- ✅ 자동 검증 완료 항목:
  - 백엔드 테스트 ($BACKEND_TEST_CMD): {결과}
  - 타겟 API 검증: {결과}
  - Playwright 타겟 검증: {결과}

- ⬜ 수동 검증 필요 항목:
  - `$BACKEND_RUN_CMD` 또는 `docker compose up --build` (CONTAINER_RUNTIME=docker) (코드 반영)
  - {기타 수동 검증 항목}
```
