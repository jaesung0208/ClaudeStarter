...
## 자동 검증 결과
- 백엔드 테스트 ($BACKEND_TEST_CMD): {통과 / 실패 — 실패 시 케이스 목록}
- API 검증: {통과 / 실패}
- Playwright UI: {통과 / 실패 — 실패 시 스크린샷 경로}

## 수동 검증 항목
- `$BACKEND_RUN_CMD` 또는 컨테이너 재실행 (CONTAINER_RUNTIME=docker 시 docker compose up --build): ⬜ 미완료 (개발자 수행 필요)
...
- 테스트 실패 시: 실패한 테스트 케이스 목록을 보고하고 사용자에게 수정 여부를 확인합니다.
