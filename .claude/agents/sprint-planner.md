...
| 2 | 보안, 인증, 암호화, 개인정보, ISMS, API Key, JWT, 권한 | `secure-coding` | 인증 API 구현, 개인정보 처리, 비밀번호 암호화 |
| 3 | UI 컴포넌트, 페이지, 화면, 디자인, 프론트엔드 구현 | `frontend-design` | 대시보드 UI 구현, 폼 컴포넌트 작성 |
| 4 | 설계 대안 비교, "A vs B", 방법 선택, 아키텍처 결정 | `brainstorming` | 캐시 전략 선택, 인증 방식 비교 |
| 5 | 위 해당 없음 | (생략) | 일반 API 구현, DB 마이그레이션 |

**자동 배정 결과 형식** (sprint{n}.md Task 항목에 반영):
```markdown
- ⬜ 로그인 버그 수정 · skill: systematic-debugging
- ⬜ JWT 인증 API 구현 · skill: secure-coding
