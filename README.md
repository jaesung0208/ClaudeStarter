...
## 📖 사용법이 궁금하다면 — 실전 워크플로우 가이드

셋업 후 실제로 어떻게 사용하는지, 기능 추가는 어떻게 하는지, 중간에 이슈가 생기면 어떻게 대응하는지가 궁금하시다면:

### 👉 [`docs/workflow-guide.md`](docs/workflow-guide.md) — 실전 사용 가이드

이 문서에서 다루는 내용:

| 섹션 | 내용 |
|---|---|
| **전체 워크플로우** | PRD → ROADMAP → Sprint → Deploy 흐름도 |
| **새 프로젝트 시작** | 4단계 가이드 (클론 → SETUP → PRD → Sprint) |
| **기능 추가하기** | 작은/중간/큰 기능별 3가지 케이스 |
| **PRD.md 관리** | 폴더 구조 진화 + 분할 시점 + 분할 방법 |
| **상황별 대응** ⭐ | 버그·새 요구사항·hook 차단·우선순위 변경 등 8가지 시나리오 |
| **브랜치 전략 + 머지** | `sprint{n}` → `develop` → `main` |
| **에이전트 호출 방법** | 8개 에이전트별 호출 예시 |
| **FAQ + 프롬프트 팁** | 자주 묻는 질문 + AI 대화 잘하는 방법 |

신규 팀원 온보딩 또는 6개월 후 본인이 다시 볼 때 참조하시면 됩니다.
