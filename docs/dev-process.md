...
> ⚠️ **명령어 예시 안내**: 이 문서의 명령어 예시는 `CONTAINER_RUNTIME=docker`, `DB_TYPE=postgres`,
> `BACKEND_STACK=python-*` 기준입니다. 다른 스택 사용 시 `.claude/project-spec.conf`의
> `BACKEND_TEST_CMD`, `BACKEND_RUN_CMD`, `DB_MIGRATION_TOOL` 등의 변수에 맞게 적절히 대체하세요.
> 예: `pytest` → `pnpm test` / `./gradlew test` / `dotnet test`
> 예: `docker compose` → `podman-compose` / 직접 프로세스 실행
> 예: `alembic` → `prisma migrate` / `flyway` / `ef core migrations`
...
**백엔드 테스트 실패 시**
1. 로컬에서 `$BACKEND_TEST_CMD` (project-spec.conf의 백엔드 테스트 명령) 실행하여 실패 케이스 확인
2. 실패 테스트를 개별 실행: `$BACKEND_TEST_CMD` 개별 테스트 옵션 (예: `pytest tests/test_foo.py::test_bar`, `pnpm test foo.spec`, `dotnet test --filter ...`)
3. 수정 후 다시 push — CI가 자동 재실행됨

**pnpm test 실패 시**
1. 로컬에서 `$FRONTEND_TEST_CMD` 실행하여 실패 케이스 확인
