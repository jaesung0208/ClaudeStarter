...
| 1.2 | 컨테이너 상태 확인 | `ssh {USER}@{SERVER_IP} "컨테이너 명령 (예: `docker compose`, CONTAINER_RUNTIME에 따라) -f /opt/app/docker-compose.prod.yml ps"` | 모든 서비스 `running` |
| 1.3 | 백엔드 에러 로그 확인 | `ssh {USER}@{SERVER_IP} "컨테이너 명령 (예: `docker compose`, CONTAINER_RUNTIME에 따라) -f /opt/app/docker-compose.prod.yml logs backend --tail 30"` | `ERROR`/`TRACEBACK`/`CRITICAL` 없음 |
| 1.4 | DB 마이그레이션 상태 | `ssh {USER}@{SERVER_IP} "컨테이너 명령 (예: `docker compose`, CONTAINER_RUNTIME에 따라) -f /opt/app/docker-compose.prod.yml exec backend alembic current"` | `(head)` 상태 |
